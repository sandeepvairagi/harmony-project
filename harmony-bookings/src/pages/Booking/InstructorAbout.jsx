...
const InstructorAbout = ({name, about, qualifications, experiences}) => {
  return (
    <div>
        <div>
            {/* <h3 className='text-2xl leading-[30px] text-headingColor font-semibold flex items-center gap-2 '
            > 
            <span className=' text-[#5D59D9] font-bold text-2xl leading-9 ' >{name}</span>
            </h3> */}
        ...
        <div className='mt-12' >
            ...
            <ul className='pt-4 md:p-5 ' >
            {
                qualifications?.map((item,index)=>
                    ...
                    <div>
                        <span className=' text-[#5D59D9] text-[15px] leading-6 font-semibold ' >
            ...
            <ul className='grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5 ' >
                {
                    experiences?.map((item, index) =>(
                        ...
                        <span className=' text-[#5D59D9] text-[15px] leading-6 ' >
                            {formateDate(item.startingDate)} - {formateDate(item.endingDate)}
                        </span>
                        <p className='text-[16px] leading-6 font-medium text-textColor '
                            >{item.position}</p>
